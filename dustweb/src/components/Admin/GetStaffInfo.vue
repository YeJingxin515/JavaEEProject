<template>
  <!--查看单个处理员的界面-->
  <el-dialog title="您查找的处理员信息" v-model="dialogVisible">
    <el-form>
      <el-form-item label="监察者编号" :label-width="formLabelWidth">
        <span>{{ idData.id }}</span>
      </el-form-item>
  ...
  <!--搜索功能-->
  <el-form :inline="true">
    <el-form-item label="查询处理员">
      <el-autocomplete
        placeholder="请输入处理员编号"
        v-model="ID"
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" @click="searchID(ID)"
          >搜索</el-button
        >
      </el-form-item>
    </el-form-item>
  ...
  <el-dialog :model="form" title="修改该用户密码" v-model="editPasswordVisible">
    <el-form :model="form" ref="editPassForm">
      <el-form-item label="密码" :label-width="formLabelWidth">
        <el-input v-model="idData.password" clearable></el-input>
      </el-form-item>
      <el-form-item>
  ...
  <el-dialog title="修改该用户个人信息" v-model="editInfoVisible">
    <el-form :model="editForm" ref="editInfoFormRef">
      ...
      <el-form-item label="工作地点" :label-width="formLabelWidth">
        <el-input v-model="idData.plantname" clearable></el-input>
      </el-form-item>
      <el-form-item>
  ...
  >
    ...
    <el-table-column label="处理员编号" prop="id"> </el-table-column>
    <el-table-column label="姓名" prop="name"> </el-table-column>
    <el-table-column label="电话" prop="phonenumber"> </el-table-column>
    <el-table-column label="工作地点" prop="plantname"> </el-table-column>
...
export default {
  components: { defineComponent, onMounted },
  data() {
    return {
  ...
  methods: {
    ...
    //获取所有投放者信息
    getData() {
      const url = this.$URL + "/User/GetInformation/AllStationStaff";
      fetch(url, {
        method: "GET",
        headers: {
          accept: "text/plain",
    ...
    submitForm() {
      ...
      const req = {
        ...
        plantname: this.idData.plantname,
    ...
    searchID(id) {
      ...
      fetch(this.$URL + "/User/GetInformation/StationStaff?req=" + id, {
        method: "GET",
        headers: {
          Authorization:
            "Bearer " + Base64.decode(localStorage.getItem("token")),
