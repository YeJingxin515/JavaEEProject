<template>
  <!--查看单个运输员的界面-->
  <el-dialog title="您查找的监察者信息" v-model="dialogVisible">
    <el-form>
  ...
  <!--搜索功能-->
  <el-form :inline="true">
    <el-form-item label="查询运输员">
      <el-autocomplete
        placeholder="请输入运输员编号"
        v-model="ID"
  ...
  >
    ...
    <el-table-column label="运输员编号" prop="id"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData() {
      const url = this.$URL + "/User/GetInformation/AllCarrier";
      fetch(url, {
        method: "GET",
        headers: {
          accept: "text/plain",
    ...
    searchID(id) {
      ...
      fetch(this.$URL + "/User/GetInformation/Carrier?req=" + id, {
        method: "GET",
        headers: {
          Authorization:
            "Bearer " + Base64.decode(localStorage.getItem("token")),
